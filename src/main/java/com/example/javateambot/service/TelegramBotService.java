...
public class TelegramBotService {
    ...
    public String rulesForDating() {
        return " Правила знакомства с собакой: " +
                "Спросите разрешения у сотрудников приюта перед тем, как подойти к собаке. Они могут дать вам дополнительную информацию о поведении собаки и помочь с выбором подходящего питомца.\n" +
                "\n" +
                "Подойдите к клетке собаки медленно и тихо, чтобы не пугать ее. Если собака выглядит напуганной или злой, не пытайтесь подходить к ней.\n" +
                "\n" +
                "Разговаривайте с собакой мягким и успокаивающим голосом, чтобы она могла узнать ваш голос и начать доверять вам.\n" +
                "\n" +
                "Если собака показывает интерес к вам, вы можете попросить сотрудника приюта разрешения на прогулку с ней в безопасной зоне.\n" +
                "\n" +
                "Во время прогулки следуйте инструкциям сотрудника приюта и не позволяйте собаке подходить к другим собакам или людям  ";
    }
    ...
    public String listOfDocuments() {
        return "Чтобы забрать собаку из приюта необходимо приготовить следующий пакет документов:\n" +
                ...
                "2. Документ, подтверждающий право собственности на жилье (квартиру или дом),\n " +
                "в котором будет проживать собака. Это может быть, например,\n" +
                " копия свидетельства о праве собственности или договор аренды жилья.\n" +
                "3. Документ, подтверждающий финансовую способность содержать собаку. \n" +
                "Это может быть выписка из банка или трудовой договор, который подтверждает достаточный уровень дохода." +
                ...
                "6. Подписанный договор на усыновление собаки из приюта, который подписывается между приютом и новым " +
                "владельцем собаки\n";
    ...
    public String recommendationsForTransportation() {
        return "Для комфортной транспортировки животного необходимо:\n" +
                " 1.Подходящий контейнер: животное должно находиться в контейнере, который соответствует его размеру," +
                " весу и виду. Контейнер должен быть достаточно прочным,  " + "и достаточно вентилируемым, " +
                "чтобы обеспечить животному достаточный доступ к свежему воздуху.\n" +

                "2. Безопасность: контейнер должен быть безопасным для животного. \n" +
                "Для этого необходимо убедиться, что в контейнере нет острых предметов или других опасных предметов,\n " +
                "которые могут нанести вред животному.\n" +

                "3. Комфорт: животное должно находиться в комфортных условиях. \n" +
                "Необходимо обеспечить достаточный доступ к воде и пище,\n " +
                "а также мягкую подстилку или коврик для обеспечения комфорта животному во время транспортировки.\n";
    }
    ...
    public String homeImprovementForAdultDog() {
        return "Для обустройства дома взрослой собаки необходимо:\n" +
                " Место для отдыха: Взрослая собака должна иметь свое место для отдыха, где она может спокойно спать и отдыхать. Это может быть специальная корзина, кровать или подушка.\n" +
                "\n" +
                "Место для еды и воды: Взрослая собака должна иметь место, где она будет кушать и пить воду. Это может быть специальный кормушка или миска.\n" +
                "\n" +
    ...
    public String TipsFromDogHandler() {
        return "Наши кинологи рекоммендуют:\n" +
                "Подойдите к собаке медленно и осторожно, не бегите и не приближайтесь слишком близко.\n" +
                "\n" +
                "Дайте собаке возможность познакомиться с вами, представьтесь ей голосом и позвольте ей нюхать вас.\n" +
                "\n" +
                "Не приставайте к собаке, если она не хочет вас видеть. Если она отходит от вас или ворчит, уйдите в сторону и не настаивайте на общении.\n" +
                "\n" +
                "Никогда не подходите к собаке сзади и не трогайте ее, пока она не познакомится с вами и не даст вам свое разрешение.\n" +
                "\n" +
                "Никогда не поднимайте и не обнимайте собаку без ее разрешения.\n" +
                "\n" +
                "Избегайте прямого взгляда и не глазируйте на собаку, это может быть воспринято ею как угроза.\n" +
                "\n" +
                "Говорите с собакой мягко и дружелюбно, используйте ее имя и обращайтесь к ней уважительно.\n" +
                "\n" +
                "Если вы хотите погладить собаку, позвольте ей подойти к вам самой и выразить свое желание получить ласку.\n" +
                "\n" +
                "Никогда не кричите на собаку и не пугайте ее, это может привести к негативным последствиям.\n" +
                "\n" +
                "Если у собаки есть владелец, обязательно спросите его разрешения, прежде чем общаться со собакой.";
    }
    ...
    public String ReasonsForRefusal() {
        return "Причины отказа:\n" +
                ...
                "Несоответствие требованиям: Например,  могут потребовать от вас доказательства того, " +
                "что вы живете в доме с огороженным двором, чтобы собака не убежала. Или же, может быть, они не " +
                "разрешают выдачу животных людям, которые ранее были лишены права на содержание животных.\n" +
                "\n" +
                "Несоответствие характера собаки: Некоторые собаки могут иметь поведенческие проблемы, " +
                "такие как агрессивность или страх, и приют может не считать вас подходящим владельцем для такой собаки." +
                " Приют может также не выдавать животных людям, которые не имеют опыта владения животными или не могут " +
                "предоставить достаточное количество времени и внимания для ухода за собакой.\n" +
                "\n" +
                "Ограниченный запас: Если приют не имеет достаточного количества собак " +
                "или если они имеют очень высокий спрос, они могут отказать вам в выдаче собаки.\n";
